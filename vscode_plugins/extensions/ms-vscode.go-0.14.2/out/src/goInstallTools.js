    ...
    return missing
        ...
        return res.then((sofar) => new Promise((resolve, reject) => {
            ...
            const callback = (err, stdout, stderr) => {
                ...
                const importPath = goTools_1.getImportPathWithVersion(tool, tool.version, goVersion);
                if (err) {
            ...
            closeToolPromise.then((success) => {
                ...
                cp.execFile(goRuntimePath, args, opts, (err, stdout, stderr) => {
                    if (stderr.indexOf('unexpected directory layout:') > -1) {
                        goStatus_1.outputChannel.appendLine(`Installing ${importPath} failed with error "unexpected directory layout". Retrying...`);
        ...
        if (failures.length === 0) {
            if (goTools_1.containsString(missing, 'gopls')) {
